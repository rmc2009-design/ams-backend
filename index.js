...
async function insertBlockFromTemplate(dayId, templateId, label, order, type) {
  try {
    ...
    if (exR.error || !exR.data.length) { console.error('Template exercises error:', exR.error ? exR.error.message : 'no exercises'); return; }
    ...
    for (var i = 0; i < exR.data.length; i++) {
      ...
      if (exIns.error) { console.error('Exercise insert error:', exIns.error.message); continue; }
...
app.get('/api/suggest/:athleteId', async function(req, res) {
  try {
    ...
    var trackWord = (a.recommended_track || 'hip_ir').replace('hip_', '').replace('_', ' ');
    ...
    res.json({
      track: a.recommended_track, pathway: a.prescribed_pathway,
      phase: a.current_phase, warmup_suggestions: warmups, block1_suggestions: block1,
    });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
