  ...
  try {
    ...
    var warmups = templates.data.filter(function(t) {
      return t.template_type === 'warmup' &&
        (t.name.toLowerCase().includes(trackWord) ||
         (t.focus_area && t.focus_area.toLowerCase().includes(trackWord)));
    });
...
function getHTML() {
  var css = [
    ...
    '.g { background:#dcfce7; color:#166634; } .bl { background:#dbeafe; color:#1e40af; } .gr { background:#f1f5f9; color:#475569; }',
    ...
    '.ok { background:#f0fdf4; border:1px solid #86efac; color:#166634; }',
    ...
    '.pass { color:#166634; font-weight:600; } .fail { color:#991b1b; font-weight:600; }',
  ...
  var pages = [
    ...
    '<div class="form-group"><label>ADD ROM</label><select id="as-add-rom" class="sel" style="margin-bottom:0"><option value="">Select...</option><option value="true">Pass</option><option value="false">Fail</option></select></div>',
    '</div>',
    ...
    '</div>',
    '<div class="form-row">',
    '<div class="form-group"><label>1080 Asymmetry (%)</label><input type="number" id="as-1080-asym" class="inp" placeholder="L/R difference" step="0.1"></div>',
    '<div class="form-group"><label>Dominant Side</label><select id="as-1080-side" class="sel" style="margin-bottom:0"><option value="">Select...</option><option value="left">Left</option><option value="right">Right</option><option value="none">None</option></select></div>',
    '</div>',
    ...
    '<div class="form-group"><label>Current Phase</label><select id="as-phase" class="sel" style="margin-bottom:0"><option value="">Select...</option><option value="1">Phase 1</option><option value="2">Phase 2</option><option value="3">Phase 3</option><option value="4">Phase 4 - Peaking</option></select></div>',
    '</div>',
    ...
    // New program form
    '<div class="card" id="new-program-form" style="display:none">',
    '<h3>New Program</h3>',
    '<div class="form-row">',
    ...
    '<div class="form-group"><label>Weeks</label><select id="np-weeks" class="sel" style="margin-bottom:0"><option value="4">4 weeks</option><option value="6">6 weeks</option><option value="8">8 weeks</option></select></div>',
    '</div>',
  ...
  var js = [
    ...
    'var h="";for(var i=0;i<fl.length;i++){var f=fl[i];var nm=f.athletes?f.athletes.first_name+" "+f.athletes.last_name:"Unknown";var sc=f.severity=="critical"?"r":f.severity=="warning"?"a":"bl";h+="<div class=\"fi\" id=\"fg"+f.id+"\"><span class=\"rb\" onclick=\"rf("+JSON.stringify(f.id)+")\">Resolve</span><div class=\"fn\">"+nm+" "+bx(f.severity,sc)+"</div><div class=\"fm\">"+f.message+"</div><div class=\"ft\">"+ta(f.created_at)+"</div></div>";}',
    'el.innerHTML=h;});}',
    ...
    'var el=document.getElementById("sc");if(!data.length){el.innerHTML="<div class=\"em\">No 1080 data.</div>";return;}',
    ...
    'el.innerHTML=h+"</tbody></table>";});',
    'if(aid){document.getElementById("norm-card").style.display="block";fetch("/api/norms?athlete_id="+aid).then(function(r){return r.json();}).then(function(norms){var nc=document.getElementById("norm-content");if(!norms.length){nc.innerHTML="<div class=\"em\">No norms yet.</div>";return;}var h="";for(var i=0;i<norms.length;i++){var n=norms[i];h+="<div style=\"padding:8px 0;border-bottom:1px solid #f1f5f9;display:flex;gap:16px;font-size:13px\"><strong style=\"min-width:120px\">"+n.exercise+"</strong><span style=\"color:#64748b;min-width:100px\">"+n.direction+"</span><span>Mean: <strong>"+parseFloat(n.mean).toFixed(1)+"%</strong></span><span>SD: <strong>"+parseFloat(n.std_dev).toFixed(1)+"%</strong></span><span style=\"color:#64748b\">"+n.session_count+" sessions</span></div>";}nc.innerHTML=h;});}else{document.getElementById("norm-card").style.display="none";}}',
    ...
    'if(!data.length){h+="<div class=\"em\">No programs yet.</div>";}else{',
    'for(var i=0;i<data.length;i++){var p=data[i].programs||data[i];h+="<div class=\"prog-item\" onclick=\"loadProgramDetail("+JSON.stringify(p.id)+","+JSON.stringify(p.name)+")\">"+p.name+"<div style=\"font-size:11px;color:#64748b\">"+(p.weeks?p.weeks+" weeks":"")+"</div></div>";}}',
    'el.innerHTML=h;});}',
    ...
    'var h="<div style=\"padding:10px;background:#f0fdf4;border:1px solid #86efac;border-radius:6px;margin-bottom:12px;font-size:12px\">";',
    'h+="<strong>Track: "+s.track.replace(/_/g," ").toUpperCase()+"</strong>";',
    ...
    'for(var i=0;i<s.warmup_suggestions.length;i++){var t=s.warmup_suggestions[i];h+="<div class=\"suggest-item\" id=\"st-wu-"+i+"\" onclick=\"pickTemplate(this,"+JSON.stringify(t.id)+","+JSON.stringify(t.name)+",\"wu\")\">"+t.name+"</div>";}',
    '}',
    'if(s.block1_suggestions&&s.block1_suggestions.length){',
    'h+="<div style=\"font-size:11px;font-weight:700;color:#64748b;margin:10px 0 6px\">SUGGESTED BLOCK 1</div>";',
    'for(var i=0;i<s.block1_suggestions.length;i++){var t=s.block1_suggestions[i];h+="<div class=\"suggest-item\" id=\"st-b1-"+i+"\" onclick=\"pickTemplate(this,"+JSON.stringify(t.id)+","+JSON.stringify(t.name)+",\"b1\")\">"+t.name+"</div>";}',
    '}',
    'sb.innerHTML=h;',
    'window._suggestions=s;',
    '});}',

    'window._picks={};',
    'function pickTemplate(el,id,name,slot){',
    'window._picks[slot]=id;',
    'var items=document.querySelectorAll(".suggest-item");',
    'for(var i=0;i<items.length;i++){if(items[i].id&&items[i].id.indexOf("st-"+slot)===0)items[i].classList.remove("selected");}',
    ...
    'var m=document.getElementById("np-result");m.style.display="block";m.className="mt";m.textContent="Creating...";',
    ...
    'return fetch("/api/programs/"+prog.id+"/assign",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({athlete_id:aid})})',
    '.then(function(){',
    'm.className="mt ok";',
    'm.textContent="Program created! Select it from the list to view and add exercises.";',
    'loadPrograms();',
    'document.getElementById("new-program-form").style.display="none";',
    '});',
    '}).catch(function(e){m.className="mt er";m.textContent="Failed: "+e.message;});}',
    ...
    'var h="";',
    'for(var d=0;d<days.length;d++){var day=days[d];h+="<div style=\"margin-bottom:24px\"><div class=\"day-hdr\">"+day.day_name+"</div>";',
    'var blocks=day.workout_blocks||[];blocks.sort(function(a,b){return a.block_order-b.block_order;});',
    'for(var b=0;b<blocks.length;b++){var block=blocks[b];var exs=block.workout_exercises||[];exs.sort(function(a,b){return a.exercise_order-b.exercise_order;});if(!exs.length)continue;',
    'h+="<div class=\"blk-lbl\">Block "+block.block_label+"</div><table><thead><tr><th>Exercise</th><th>Sets</th><th>Tempo</th><th>Rest</th><th>Reps Wk"+week+"</th><th>Weight</th></tr></thead><tbody>";',
    ...
    'h+="</tbody></table>";}h+="</div>";}',
    'document.getElementById("program-days").innerHTML=h||"<div class=\"em\">No exercises yet. This feature is coming soon.</div>";});}',
    ...
    'fetch("/import/1080",{method:"POST",body:fd}).then(function(r){return r.json();}).then(function(d){if(d.error){m.className="mt er";m.textContent="Error: "+d.error;}else{m.className="mt ok";m.textContent=d.saved+" records imported. "+d.skipped+" skipped.";}}).catch(function(e){m.className="mt er";m.textContent="Failed: "+e.message;});}',

    'ld();',
    '</script>'
  ].join('');

  return '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>AMS</title>' + css + '</head><body>'
    + '<div class="top"><h1>Athlete Management System</h1><span id="upd" style="font-size:12px;color:#94a3b8">Loading...</span></div>'
    + nav + pages + js + '</body></html>';
}
