...
function getHTML() {
  ...
  var pages = '<div class="wrap">'
    ...
    + '<div class="form-row"><div class="form-group"><label>Date</label><input type="date" id="as-date" class="inp"></div>'
    + '<div class="form-group"><label>Bodyweight (kg)</label><input type="number" id="as-bw" class="inp" placeholder="e.g. 95"></div></div>'
    + '<div style="font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase;margin:10px 0 6px">Force Plate (Hawkin)</div>'
    + '<div class="form-row"><div class="form-group"><label>Braking Net Impulse</label><input type="number" id="as-bni" class="inp" placeholder="threshold 1.5" step="0.01"></div>'
    + '<div class="form-group"><label>Propulsive Net Impulse</label><input type="number" id="as-pni" class="inp" placeholder="threshold 2.8" step="0.01"></div></div>'
    + '<div class="form-row"><div class="form-group"><label>Peak Eccentric Speed (m/s)</label><input type="number" id="as-pes" class="inp" placeholder="threshold 1.5" step="0.01"></div>'
    + '<div class="form-group"><label>Takeoff Velocity (m/s)</label><input type="number" id="as-tv" class="inp" placeholder="threshold 2.75" step="0.01"></div></div>'
    + '<div class="form-row"><div class="form-group"><label>P1 Impulse</label><input type="number" id="as-p1" class="inp" step="0.001"></div>'
    + '<div class="form-group"><label>P2 Impulse</label><input type="number" id="as-p2" class="inp" step="0.001"></div></div>'
    + '<div style="font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase;margin:10px 0 6px">Force Frame</div>'
    + '<div class="form-row"><div class="form-group"><label>IR Force (N)</label><input type="number" id="as-ir-ff" class="inp" placeholder="threshold 200N"></div>'
    + '<div class="form-group"><label>IR Asymmetry (%)</label><input type="number" id="as-ir-asym" class="inp" placeholder="threshold 10%"></div></div>'
    + '<div class="form-row"><div class="form-group"><label>ADD Straight Left (N)</label><input type="number" id="as-add-sl" class="inp"></div>'
    + '<div class="form-group"><label>ADD Straight Right (N)</label><input type="number" id="as-add-sr" class="inp"></div></div>'
    + '<div class="form-row"><div class="form-group"><label>ADD Bent Total (N)</label><input type="number" id="as-add-bt" class="inp" placeholder="threshold 400N"></div>'
    + '<div class="form-group"><label>ADD ROM</label><select id="as-add-rom" class="sel" style="margin-bottom:0"><option value="">Select...</option><option value="true">Pass</option><option value="false">Fail</option></select></div></div>'
...
h+="<div class=\'pathway-box\'><div class=\'pathway-title\'>"+boxTitle+"</div><div class=\'pathway-desc\'>"+boxDesc+"</div></div>";
    ...
    + 'if(a.p1_p2_ratio!=null){h+="<div class=\'gate-item\'>"+(a.p1_p2_ratio<1.3?"<span class=\'pass\'>":"<span class=\'fail\'>")+parseFloat(a.p1_p2_ratio).toFixed(3)+"</span> P1/P2 Ratio (timing issue if >= 1.3)</div>";}'
    + 'h+="</div>";}'
    + 'el.innerHTML=h;});}'
    ...
    + 'var h="";for(var i=0;i<data.length;i++){var p=data[i].programs||data[i];h+="<div class=\'prog-item\' onclick=\'loadProgramDetail("+JSON.stringify(p.id)+","+JSON.stringify(p.name)+")\' ><div style=\'font-weight:600;font-size:13px\'>"+p.name+"</div><div style=\'font-size:12px;color:#64748b\'>"+(p.phase||"")+(p.weeks?" - "+p.weeks+" weeks":"")+"</div></div>";}'
    + 'el.innerHTML=h;});}'
